...
# compléter ci-dessous une itération de Gauss Seidel
# Les iteration stoppent lorsque la différence entre deux itérés est 
# plus petite que tol. Le nombre d'itération ne doit pas exéder max_iter
...
# -----------------------------------------------------------------------------
# Caclul de T^(n+1) de maniere implicite
# -----------------------------------------------------------------------------
...
# compléter le membre de droite  au temps t_{n+1}
...
# appel à fonction qui inverse par Gauss Seidel.
# Pour diminuer les itérations on les commencent avec Tn

    Tnp1,itergs = solve_GS(Tn, rhs, Dx_coeff, Dy_coeff)
    return Tnp1,itergs
...
# Affichage du nombre d'itération de GS pour converger à chaque pas de temps
